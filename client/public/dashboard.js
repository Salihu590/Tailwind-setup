...
const pendingPaymentsMetric = document.getElementById("pending-payments-metric");
const ordersToShipMetric = document.getElementById("orders-to-ship-metric");

const searchInput = document.getElementById("search-input");
const paymentStatusFilter = document = document.getElementById("payment-status-filter");
...
const clearCustomerFiltersButton = document.getElementById("clear-customer-filters");
...
const revenueReportContainer = document.getElementById("revenue-report-container");
const topProductsReportContainer = document.getElementById("top-products-report-container");

const topProductsList = document.getElementById("top-products-list");

const allSections = [ordersSection, customersSection, revenueReportContainer, topProductsReportContainer];
const allButtons = [ordersViewBtn, customersViewBtn, revenueViewBtn, topProductsViewBtn];
...
ordersViewBtn.addEventListener("click", () => showSection(ordersSection, ordersViewBtn));
...
async function renderDashboard() {
  ...
  if (token) {
    ...
    try {
      ...
      const ordersToShip = orders.filter(order => order.orderStatus === 'confirmed').length;
      ...
      const filteredOrders = orders.filter(order => {
        ...
        const matchesPaymentStatus = selectedPaymentStatus === '' || order.paymentStatus.toLowerCase() === selectedPaymentStatus;
        const matchesOrderStatus = selectedOrderStatus === '' || order.orderStatus.toLowerCase() === selectedOrderStatus;
...
async function renderCustomers() {
    ...
    try {
        ...
        const filteredCustomers = allCustomersData.filter(customer => {
            ...
            return fullName.includes(searchQuery) || customer.email.toLowerCase().includes(searchQuery);
        });
        ...
        customer.orders.forEach(order => {
             ordersHtml += `
                <div class="order-card" style="margin-bottom: 15px;">
                    ...
                    <p><strong>Status:</strong> <span class="status-badge ${order.orderStatus}">${order.orderStatus}</span></p>
                </div>
            `;
        });
...
function populateOrderModal(order) {
  orderDetailsContent.innerHTML = `
        <h3>Order ID: ${order.orderId}</h3>
        <p><strong>Customer:</strong> ${order.checkoutData.firstName} ${order.checkoutData.lastName}</p>
        <p><strong>Email:</strong> ${order.checkoutData.email}</p>
        <p><strong>Phone:</strong> ${order.checkoutData.phone}</p>
        ...
        <p><strong>Apartment/Suite:</strong> ${order.checkoutData.apartment || 'N/A'}</p>
        <p><strong>Total:</strong> ₦${order.total.toFixed(2)}</p>
        <p><strong>Payment Status:</strong> <span class="status-badge ${order.paymentStatus.toLowerCase()}">${order.paymentStatus}</span></p>
        <p><strong>Order Status:</strong> <span class="status-badge ${order.orderStatus.toLowerCase()}">${order.orderStatus}</span></p>
        <hr>
        <h4>Items:</h4>
        <ul>
            ${order.cartItems
        ...
        <div class="add-note-container">
            ...
            <button id="add-note-button" class="action-button" onclick="addOrderNote('${order._id}')">Add Note</button>
        </div>
    `;
}
